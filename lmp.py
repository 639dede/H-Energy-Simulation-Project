...
class JPN_DA_LMP_determiner(pyo.ConcreteModel):

    def __init__(self, demand_data, generator_data, connections, fmax_data):
        ...
        self.fmax_data = fmax_data
        
        self.all_generators = []
        
        for region in self.generator_data:
            for gen_id in self.generator_data[region]:
                self.all_generators.append(gen_id)
    ...
    def _generate_model(self):
        ...
        model.G = pyo.Set(initialize=self.all_generators, doc="All generators")
        
        def get_region_generators(model, i):
            """지역 i의 발전기 집합 G_i"""
            return [gen_id for gen_id in self.generator_data.get(i, {})]
        
        model.G_i = pyo.Set(model.N, initialize=get_region_generators, doc="Generators in each region")
        
        model.L = pyo.Set(initialize=self.connections, doc="Transmission line connections")
        
        # 파라미터 정의
        model.d = pyo.Param(model.N, initialize=self.demand_data, doc="Demand at each region (MW)")
        
        def gen_cost_init(model, g):
            for region in self.generator_data:
                if g in self.generator_data[region]:
                    return self.generator_data[region][g]['cost']
            return 0
        ...
        for i in model.N:
            ...
            for g in generators_in_region:
                ...
                for region in self.generator_data:
                    if g in self.generator_data[region]:
                        gen_type = self.generator_data[region][g]['type']
                        break
        ...
        (results.solver.termination_condition == TerminationCondition.optimal):
            ...
            if active_gens:
                ...
                for idx, (g, cost, generation) in enumerate(active_gens[:10], 1):
                    ...
                    region = None
                    gen_type = None
                    for r in self.generator_data:
                        if g in self.generator_data[r]:
                            region = r
                            gen_type = self.generator_data[r][g]['type']
                            break
...
# =============================================================================
# Infeasibility diagnose object
# =============================================================================

class JPN_DA_LMP_diagnose_infeasibility(pyo.ConcreteModel):
    ...
    def _generate_model(self):
        ...
        # 집합 정의
        model.N = pyo.Set(initialize=regions, doc="Regions")
        ...
        model.G_i = pyo.Set(model.N, initialize=get_region_generators, doc="Generators in each region")
        model.L = pyo.Set(initialize=self.connections, doc="Transmission lines")

        # 파라미터 정의
        model.d = pyo.Param(model.N, initialize=self.demand_data, doc="Demand")
        ...
        model.c_g = pyo.Param(model.G, initialize=gen_cost_init, doc="Generation cost")
        ...
        model.p_g_max = pyo.Param(model.G, initialize=gen_capacity_init, doc="Generator capacity")
        
        def fmax_init(model, i, j):
            return self.fmax_data.get(i, {}).get(j, 0)
        
        model.f_max = pyo.Param(model.L, initialize=fmax_init, doc="Line capacity")

        # 변수 정
        model.p_g = pyo.Var(model.G, bounds=lambda m, g: (0, m.p_g_max[g]))
        model.f = pyo.Var(model.L, bounds=lambda m, i, j: (0, m.f_max[i,j]))

        # Slack variables 추가
        model.s_pos = pyo.Var(model.N, bounds=(0, None), doc="Positive slack (demand shortage)")
        model.s_neg = pyo.Var(model.N, bounds=(0, None), doc="Negative slack (supply excess)")


        ## 2. 목적함수 : slack variables의 합을 최소화
        
        model.obj = pyo.Objective(
            expr=sum(model.s_pos[i] + model.s_neg[i] for i in model.N),
            sense=pyo.minimize,
            doc="Minimize total slack"
        )


        ## 3. 제약 조건 : 수정된 수급 균형 제약조건
        ...
        model.slack_power_balance = pyo.Constraint(model.N, rule=slack_power_balance_rule)
    ...
    def diagnose(self):
        ...
        (diag_results.solver.termination_condition == TerminationCondition.optimal):
            ...
            if total_slack < 1e-6:
                ...
                for i in model.N:
                    s_pos_val = pyo.value(model.s_pos[i])
                    s_neg_val = pyo.value(model.s_neg[i])
...
if __name__ == "__main__":
    ...
    LMP_model = JPN_DA_LMP_determiner(
        demand_data, 
        generator_data, 
        connections, 
        fmax_data
    ...
    LMP_infeas_diag = JPN_DA_LMP_diagnose_infeasibility(
        demand_data, 
        generator_data, 
        connections, 
        fmax_data
    ...
    if results is not None:
        ...
        if (results.solver.status == SolverStatus.ok) and \
           (results.solver.termination_condition == TerminationCondition.optimal):
            ...
            is_feasible = LMP_infeas_diag.diagnose()
